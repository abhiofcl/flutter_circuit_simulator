...
class NegNegClamper extends StatefulWidget {
  const NegNegClamper({Key? key}) : super(key: key);
  @override
  _NegNegClamperState createState() => _NegNegClamperState();
}

class _NegNegClamperState extends State<NegNegClamper> {
  // String _showValue = '';
  ...
  void initState() {
    super.initState();
    _apiUrl = '${Constants.apiUrl}/api/biasedclamper/4';
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Negative biased negative clamper"),
      ),
      body: Stack(
        children: [
          Column(
            ...
            children: [
              const Image(
                image:
                    AssetImage('assets/images/negative_clamper_neg_bias.png'),
              ),
              ElevatedButton(
                onPressed: () async {
                  await fetchData();
